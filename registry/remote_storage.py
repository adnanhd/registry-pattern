r"""HTTP client proxy for remote registry storage.

Requires the ``http`` extra::

    pip install 'registry-pattern[http]'
"""
...
__all__ = ["RemoteStorageProxy", "SerializationError"]
...
def _validate_namespace(namespace: str) -> str:
    ...
    if (
        ...
        raise ValidationError(detail, suggestions, {"namespace": namespace})
...
def _validate_port(port: int) -> int:
    if isinstance(port, bool):
        raise ValidationError(
            "port must be int in [1, 65535], got bool",
