...
def test_validate_instance_of_torch_device():
    ...
    device = TorchDevice.model_validate('cuda:0')._validate(torch_device)
    assert device.type == 'cuda'
    assert device.index == 0


def test_validate_instance_of_torch_device_with_no_index():
    torch_device = torch.device('cuda', 0)
    device = TorchDevice.model_validate('cuda')._validate(torch_device)
    assert device.type == 'cuda'
    assert device.index == 0


def test_validate_invalid_instance_of_torch_device():
    torch_device = torch.device('cuda', 0)
    with pytest.raises(ValidationError):
        TorchDevice.model_validate('cuda:2')._validate(torch_device)


def test_serialize_torch_device():
    torch_device = torch.device('cuda', 0)
    serialized = TorchDevice._serialize(torch_device, lambda x: x)
...
def test_validate_config_and_instance():
    ...
    assert torch_device_ins == TorchDevice.model_validate(
        torch_device_ins).build_model()
...
def test_torch_device_within_model():
    ...
    assert torch_device == TorchTensor(
        device=torch_device).device.build_model()
