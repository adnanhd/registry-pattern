...
def test_bound_to_passes_for_real_pairing() -> None:
    @_Steps.register_artifact
    def step3(
        model: nn.Module, opt: Annotated[torch.optim.Optimizer, BoundTo("model")]
    ) -> int:
...
def test_input_shape_matches_strict_compares_tensor_to_in_features() -> None:
    @_Steps.register_artifact
    def step7(
        model: nn.Module, x: Annotated[torch.Tensor, InputShapeMatches("model")]
    ) -> int:
        return x.shape[1]
...
def test_matches_input_shape_validates_against_peer_meta() -> None:
    @_Steps.register_artifact
    def step8(
        model: Any, batch: Annotated[torch.Tensor, MatchesInputShape("model")]
    ) -> int:
...
def test_matches_output_shape_links_criterion_input_to_model_output() -> None:
    @_Steps.register_artifact
    def step9(
        model: Any, criterion: Annotated[Any, MatchesOutputShape("model")]
    ) -> str:
        return "ok"
...
def test_matches_target_shape_validates_batch_target_against_criterion_meta() -> None:
    @_Steps.register_artifact
    def step10(
        criterion: Any, batch: Annotated[tuple, MatchesTargetShape("criterion")]
    ) -> str:
        return "ok"
        ...
        build(
            {
                ...
                "data": {
                    "criterion": criterion,
                    "batch": (torch.zeros(8, 2), torch.zeros(8, 1)),
                },
            }
        )
        == "ok"
